...
| Q14b | Eén kaarttekst of regel-chunk komt op zichzelf boven het embed-tekenbudget uit | De embed-invoer wordt gekapt op het budget, zodat élk verzoek binnen het gemeten veilige bereik blijft en de tekst niet elke run opnieuw een OOM-kill uitlokt. De opgeslagen tekst (`RuleChunk.Text`, kaarttekst) blijft volledig; het aantal gekapte teksten en de kaplengte staan in de run-melding — nooit stil | `EmbedBatching.CapItems`, `EmbedRunResult.Capped`, `RuleIndexResult.Capped` (#293), `EmbedOutcomeTests`, `RuleChunkPipelineTests` |
| Q14c | Iemand zet `EMBED_BATCH_CHARS` boven de gemeten veilige grens | De waarde wordt geweigerd, de pijplijn draait op de default en er gaat een expliciete waarschuwing uit — stil terugvallen zou de indruk wekken dat er bijgesteld is (de `NIGHTLY_ENABLED`-klasse fout, #268). Het plafond is de meetwaarde `MeasuredSafeMaxBatchChars`, geen ruime bovengrens | `EmbeddingSettings.FromEnvironment` (#293), `EmbedOutcomeTests` |
