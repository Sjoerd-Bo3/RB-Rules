...
/// recente Piltover Archive-decks (zie <see cref="DeckPopularityQuery.
...
public class CardDetailService(RbRulesDbContext db, CardResolver resolver)
{
    ...
    public async Task<CardDossier?> DossierAsync(string id, CancellationToken ct = default)
    {
        ...
        // Deck-gebruikssignaal (#15 golf 1 spoor B) — de query staat sinds
        // #267 op één plek (DeckPopularityQuery): het /ask-deck-meta-kanaal
        // (kennislaag 3) leest exact hetzelfde signaal.
        var deckPopularity = await DeckPopularityQuery.ForCanonicalAsync(db, canonicalId, ct);
