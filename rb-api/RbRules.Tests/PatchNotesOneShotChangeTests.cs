...
/// niet-editoriale Change is.</summary>
public class PatchNotesOneShotChangeTests
{
    [Fact]
    public void IsCandidate_PatchNotesZonderNietEditorialeChange_Kandidaat()
    {
        Assert.True(PatchNotesOneShotChange.IsCandidate(
            trustTier: 1, effectiveKind: SourceContentKind.PatchNotes, hasNonEditorialChange: false));
    }
    ...
    public void IsCandidate_PatchNotesMetNietEditorialeChange_GeenKandidaat()
    {
        ...
        Assert.False(PatchNotesOneShotChange.IsCandidate(
            trustTier: 1, effectiveKind: SourceContentKind.PatchNotes, hasNonEditorialChange: true));
    }
    ...
    {
        Assert.False(PatchNotesOneShotChange.IsCandidate(
            trustTier: 3, effectiveKind: SourceContentKind.PatchNotes, hasNonEditorialChange: false));
    }
    ...
    {
        Assert.False(PatchNotesOneShotChange.IsCandidate(
            trustTier: 1, effectiveKind: kind, hasNonEditorialChange: false));
    }
}
